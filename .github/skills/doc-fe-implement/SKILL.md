...
description: "FE screen design doc workflow for Blog Hoi An/Da Nang. Use when writing, updating, or standardizing a frontend screen specification document. Ensures consistent 9-section format with YAML frontmatter, navigation, layout tree, API links, state snippets, error handling, and responsive table. Trigger phrases: write screen doc, update screen spec, standardize doc, create design doc, doc fe, screen design, spec màn hình."
...
### Bước 1 — Xác định màn hình cần viết
1. Xem danh sách tại `demo_docs/fe/[Design][LIST] SCREEN_DanhSachManHinh.md`
2. File đích: `demo_docs/fe/[Design][SCREEN] {ScreenCode}_{ScreenName}.md`
3. Nếu màn hình gọi API, đọc trước các file API liên quan tại `demo_docs/api/`

### Bước 2 — Áp dụng 9-section template
Xem template đầy đủ tại [screen-template.md](./references/screen-template.md).

Bắt buộc có đủ 9 sections:
...
4. **Layout & Components** — JSX tree minh hoạ cấu trúc
5. **Chi tiết UI từng section** — mô tả từng block UI
